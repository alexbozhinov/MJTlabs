...
public class FootballPlayerAnalyzer {
    ...
    private final List<Player> footballPlayers;
    private static final int OVERALL_RATING_MEASURES_DIFFER_BY_AT_MOST = 3;
    ...
    public FootballPlayerAnalyzer(Reader reader) {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(reader)) {
            bufferedReader.readLine(); // only read the header line without using it

            String line;
            while ((line = bufferedReader.readLine()) != null) {
                footballPlayers.add(Player.of(line));
            }
    ...
    public List<Player> getAllPlayers() {
        ...
        return (null != footballPlayers && !footballPlayers.isEmpty()) ?
            Collections.unmodifiableList(footballPlayers) : Collections.emptyList();
    }
    ...
    public Set<String> getAllNationalities() {
        ...
        return (footballPlayers.isEmpty()) ? Collections.emptySet() : footballPlayers
            .stream()
    ...
    public Player getHighestPaidPlayerByNationality(String nationality) {
        if (null == nationality) {
            throw new IllegalArgumentException("Provided argument nationality is null!");
        }
        if (footballPlayers.stream().noneMatch(player -> player.nationality().equals(nationality))) {
            throw new NoSuchElementException("There is no player with the provided nationality!");
        }
     ...
     * overall rating, p is the player's potential and an is the player's age
     *
    ...
    public Optional<Player> getTopProspectPlayerForPositionInBudget(Position position, long budget) {
        ...
        if (null == position) {
            throw new IllegalArgumentException("Provided argument position is null!");
        }
        if (budget < 0) {
            throw new IllegalArgumentException("Provided argument budget has a negative value");
        }
        ...
        return footballPlayers
            ...
            .max(Comparator.comparing(player ->
                ((double)player.overallRating() + (double)player.potential()) / (double)player.age()));
    }
    ...
    public Set<Player> getSimilarPlayers(Player player) {
        ...
        if (null == player) {
            throw new IllegalArgumentException("Provided argument player is null!");
        }
        ...
        return footballPlayers
            ...
            .filter(player1 ->
                player1.positions().stream().anyMatch(position -> player.positions().contains(position)) &&
                    player1.preferredFoot().getFoot().equals(player.preferredFoot().getFoot()) &&
                    (Math.abs(player1.overallRating() - player.overallRating())
                        <= OVERALL_RATING_MEASURES_DIFFER_BY_AT_MOST
                    )
            )
            .collect(Collectors.toUnmodifiableSet());
    }
    ...
    public Set<Player> getPlayersByFullNameKeyword(String keyword) {
        ...
        if (null == keyword) {
            throw new IllegalArgumentException("Provided argument keyword is null!");
        }
